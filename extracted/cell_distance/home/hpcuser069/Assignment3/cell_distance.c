...
//FUNCTIONS
void calculate_distances_within_block(float ** cells_block, int nbr_cells);
void calculate_distances_between_blocks(float ** cells_block_main, float ** cells_block_sec, int nbr_cells_main, int nbr_cells_sec);
...
int main(int argc, char * argv[]){
  ...
  //printf("%d %d\n",  MAX_NBR_CELLS_IN_ARRAY, MAX_NBR_CELLS_IN_BLOCK );
  ...
  for(int block_ix=1; block_ix<=nbr_blocks; block_ix++){
    ...
    if(block_ix == nbr_blocks){
      cells_block1_entries = (float *) malloc(3*cells_in_block_rest*sizeof(float));
      cells_block1 = (float **) malloc(cells_in_block_rest*sizeof(float *));
	...
	fscanf(fp, "%f %f %f", &cells_block1[ix][0], &cells_block1[ix][1], &cells_block1[ix][2]);
      ...
      cells_block1_entries = (float *) malloc(3*cells_in_block*sizeof(float));
      cells_block1 = (float **) malloc(cells_in_block*sizeof(float *));
	...
	cells_block2_entries = (float *) malloc(3*cells_in_block_rest*sizeof(float));
	cells_block2 = (float **) malloc(cells_in_block_rest*sizeof(float *));
	for(int ix=0, jx=0; ix<cells_in_block_rest; ix++, jx+=3){
	...
	cells_block2_entries = (float *) malloc(3*cells_in_block*sizeof(float));
	cells_block2 = (float **) malloc(cells_in_block*sizeof(float *));
	for(int ix=0, jx=0; ix<cells_in_block; ix++, jx+=3){
	  cells_block2[ix] = cells_block2_entries + jx;
	}
	//PARSE
	...
	for(int ix=0; ix<cells_in_block; ix++){
      ...
      //Calculate distances
      if(block_ix == nbr_blocks){
	calculate_distances_within_block(cells_block1, cells_in_block_rest);
      } else {
	if(block2_ix == nbr_blocks){
	  calculate_distances_between_blocks(cells_block1, cells_block2, cells_in_block, cells_in_block_rest);
	} else if(block2_ix == block_ix){
	  calculate_distances_within_block(cells_block1, cells_in_block);
	} else {
	  calculate_distances_between_blocks(cells_block1, cells_block2, cells_in_block, cells_in_block);
	}
      }
      free(cells_block2);
...
#pragma omp parallel for schedule(dynamic) reduction(+:distances[:NBR_DISTANCE_POINTS_int])
  for(size_t ix=0; ix<nbr_cells; ix++){
    float ax = cells_block[ix][0];
    float ay = cells_block[ix][1];
    float az = cells_block[ix][2];
    for(size_t jx=ix+1; jx<nbr_cells; jx++){
      float dist;
      int int_dist;
      float dx = ax-cells_block[jx][0];
      float dy = ay-cells_block[jx][1];
      float dz = az-cells_block[jx][2];
      dist = dx*dx + dy*dy + dz*dz;
      dist = sqrt(dist)*10*10 + 0.5;
      int_dist = (int) dist;
      distances[int_dist]++;
